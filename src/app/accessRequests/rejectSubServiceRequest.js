...
const { getSubServiceRequestVieModel, getAndMapServiceRequest, getRoleAndServiceNames, generateFlashMessages } = require('./utils');
...
const post = async (req, res) => {
  ...
  if (model.viewModel.actioned_by && (model.viewModel.status === -1 || 1 )) {
    ...
    const { title, heading, message } = generateFlashMessages(
      'service',
      model.viewModel.status,user.claims.email,model.viewModel.endUsersGivenName,model.viewModel.endUsersFamilyName, model.viewModel.roles.map((i) => i.name),
      res,
    );
    res.flash('title', `${title}`);
    res.flash('heading', `${heading}`);
