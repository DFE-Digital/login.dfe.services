...
const signUserOut = async (req, res) => {
  if (req.user && req.user.id_token) {
    ...
    if (skipIssuerSession && isValidRedirect) {
      ...
      logger.info(
        "service signout :: there is no skipIssuer and no valid redirect",
      );
...
const isValidRedirectUrl = async (url) => {
  try {
    ...
    const getAllRedirectUrls =
      await servicePostLogoutStore.getServicePostLogoutRedirectsUrl(serviceId);
