...
const { getAllRequestsTypesForApprover } = require('../../../../src/infrastructure/organisations');
...
describe('when displaying the pending access requests for approver ', () => {
  ...
  beforeEach(() => {
    ...
    Account.getUsersById
      .mockReset()
      .mockReturnValue([
        { claims: { sub: 'user-id-1', given_name: 'User', family_name: 'One', email: 'user.one@unit.tests' } },
  ...
  it('then it should get all the access requests (organisation, service, sub-service) for all organisations where the logged-in user is Approver', async () => {
    ...
    expect(getAllRequestsTypesForApprover.mock.calls[0][3]).toBe('correlationId');
  });
  ...
  it('then it should return the `getAllRequestsForApproval` view', async () => {
    ...
    expect(res.render.mock.calls[0][0]).toBe('accessRequests/views/allRequestsForApproval');
  });

  it('then it should include the correct page title', async () => {
    await getAllRequestsForApproval(req, res);

    expect(res.render.mock.calls[0][1]).toMatchObject({ title: 'Requests - DfE Sign-in' });
  });

  it('then it should include the correct navigation', async () => {
    await getAllRequestsForApproval(req, res);

    expect(res.render.mock.calls[0][1]).toMatchObject({ currentPage: 'requests' });
  });
