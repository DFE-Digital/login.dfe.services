...
const getOrganisationPermission = require('../../../../src/app/users/organisationPermission').get;
...
describe('when displaying the organisation permission level page', () => {
  ...
  it('then it should return the organisation permission view', async () => {
    ...
    expect(res.render.mock.calls[0][0]).toBe('users/views/organisationPermission');
  });
  ...
  it('then the back link should redirect to New User page if the user don`t have a DfE Sign-in Account', async () => {
    ...
    expect(res.render.mock.calls.length).toBe(1);
    expect(res.render.mock.calls[0][1].backLink).toBe(`/approvals/${req.params.orgId}/users/new-user`);
  });
});
