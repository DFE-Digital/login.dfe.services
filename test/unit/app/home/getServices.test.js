...
const userAccess = [
  { serviceId: 'service1', organisationId: 'org1', accessGrantedOn: '2024-08-14T11:07:02Z' },
  { serviceId: 'service4', organisationId: 'org2', accessGrantedOn: '2024-06-14T11:07:02Z' },
  { serviceId: 'service3', organisationId: 'org3', accessGrantedOn: '2024-06-14T11:07:02Z' },
  { serviceId: 'service4', organisationId: 'org4', accessGrantedOn: '2024-06-14T11:07:02Z' },
];
const application = {
  id: 'service1',
  name: 'Service One',
...
describe('when displaying the users services', () => {
  ...
  beforeEach(() => {
    ...
    getOrganisationAndServiceForUser
      .mockReset()
      .mockReturnValue([
        { organisation: { id: 'org1', name: 'Organisation 1', status: { id: 1 } } },
        { organisation: { id: 'org2', name: 'Organisation 2', status: { id: 1 } } },
        { organisation: { id: 'org3', name: 'Organisation 3', status: { id: 0 } } },
        { organisation: { id: 'org4', name: 'Organisation 4', status: { id: 1 } } },
      ]);
  ...
  it('then it should include mapped services for user when one organisation is present', async () => {
    ...
    expect(res.render.mock.calls[0][1].services).toBeDefined();
    expect(res.render.mock.calls[0][1].services[0].organisations).toHaveLength(1);
  ...
  it('then it should include mapped services for user when multiple organisations are present', async () => {
    ...
    expect(res.render.mock.calls[0][1].services).toBeDefined();
    expect(res.render.mock.calls[0][1].services[1].organisations).toHaveLength(2);
  ...
  it('then it should include mapped services for user when organisation status id is 0', async () => {
    ...
    expect(res.render.mock.calls[0][1].services).toBeDefined();
    expect(res.render.mock.calls[0][1].services[2].organisations).toHaveLength(0);
  ...
  it ('then it Services should be filtered by service id', async () => {
    ...
    expect(res.render.mock.calls[0][1].services).toBeDefined();
    expect(res.render.mock.calls[0][1].services).toHaveLength(3);
    expect(res.render.mock.calls[0][1].services.filter((s) => s.id === 'service4')).toHaveLength(1);
  });
